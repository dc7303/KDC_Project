...
INSERT INTO CLASSROOM_INFO VALUES ('notice', '전체공지사항', '9999-12-31', '9999-12-31', 'ocean', 'KK');
INSERT INTO CLASSROOM_INFO VALUES ('employ', '취업공지사항', '9999-12-31', '9999-12-31', 'ocean', 'KK');


INSERT INTO NOTICE_BOARD VALUES (NOTICE_BOARD_SEQ.NEXTVAL, 'notice', '공지사항제목', 'lake', sysdate, '공지사항내용', '0', NULL, 'FALSE')
INSERT INTO NOTICE_BOARD VALUES (NOTICE_BOARD_SEQ.NEXTVAL, 'employ', '취업공지사항제목', 'lake', sysdate, '취업공지사항내용', '0', NULL, 'FALSE')
COMMIT
=======
...
, (SELECT UPDOWN_ISUP FROM UPDOWN WHERE UPDOWN_REPLY_BOARD_PK = A.REPLY_BOARD_PK AND UPDOWN_USER_ID = 'ocean') isUp
, (SELECT LISTAGG(HASHTAG_NAME, ', ') WITHIN GROUP (ORDER BY ROWNUM) FROM HASHTAG WHERE HASHTAG_REPLY_BOARD_PK = A.REPLY_BOARD_PK GROUP BY HASHTAG_REPLY_BOARD_PK) hashTagName
FROM REPLY_BOARD A, MEMBER B
WHERE A.REPLY_BOARD_CLASSIFICATION = 'tech'
AND A.REPLY_BOARD_PK = '4'
...
WHERE A.REPLY_BOARD_CLASSIFICATION = 'tech'
AND A.REPLY_BOARD_ISDELETE = 'FALSE'
AND A.REPLY_BOARD_REPLY_NO = '4'
AND A.REPLY_BOARD_WRITER_ID = B.MEMBER_ID


SELECT * FROM UPDOWN WHERE UPDOWN_REPLY_BOARD_PK = '22' AND UPDOWN_USER_ID = 'ocean'
...
SELECT * FROM REPLY_BOARD
    ...
    AND A.REPLY_BOARD_TITLE LIKE '%{}%' -- 게시판제목
    AND A.REPLY_BOARD_CONTENTS LIKE '%{}%' -- 게시판내용
    AND B.MEMBER_NICKNAME LIKE '%{}%' -- 작성자(닉네임)
    AND EXISTS (SELECT 'T'
      FROM HASHTAG C 
      WHERE C.HASHTAG_NAME LIKE '%%' -- 해시태그
    ...
    AND UPPER(A.REPLY_BOARD_TITLE) LIKE '%' || UPPER('test') || '%'

    ORDER BY A.REPLY_BOARD_PK DESC
    
    select* from reply_board
    where REPLY_BOARD_REPLY_NO is null
    and reply_board_isdelete = 'FALSE'
    AND REPLY_BOARD_TITLE LIKE '%test%'
>