--1.4변경사항
--NOTICE_BOARD 테이블 NOTICE_BOARD_CLASSIFICATION 필드 제약조건 수정
--NOTICE_BOARD 테이블 NOTICE_BOARD_CLASSROOM_CODE 필드 추가
--CLASSROOM 테이블 CLASSROOM_ISCURRENT 필드추가
...
SELECT
  (SELECT COUNT(REPLY_BOARD_PK) FROM REPLY_BOARD B 
   WHERE B.REPLY_BOARD_ISDELETE='FALSE' AND B.REPLY_BOARD_REPLY_NO IS NULL AND TO_CHAR(B.REPLY_BOARD_WRITE_DATE, 'YYYYMMDD') = TO_CHAR(SYSDATE, 'YYYYMMDD') 
   AND B.REPLY_BOARD_CLASSIFICATION = 'tech') tech
, (SELECT COUNT(REPLY_BOARD_PK) FROM REPLY_BOARD C
   WHERE C.REPLY_BOARD_ISDELETE='FALSE' AND C.REPLY_BOARD_REPLY_NO IS NULL AND TO_CHAR(C.REPLY_BOARD_WRITE_DATE, 'YYYYMMDD') = TO_CHAR(SYSDATE, 'YYYYMMDD') 
   AND C.REPLY_BOARD_CLASSIFICATION = 'lib') lib
, (SELECT COUNT(REPLY_BOARD_PK) FROM REPLY_BOARD D
   WHERE D.REPLY_BOARD_ISDELETE='FALSE' AND D.REPLY_BOARD_REPLY_NO IS NULL AND TO_CHAR(D.REPLY_BOARD_WRITE_DATE, 'YYYYMMDD') = TO_CHAR(SYSDATE, 'YYYYMMDD') 
   AND D.REPLY_BOARD_CLASSIFICATION = 'study') study
FROM DUAL